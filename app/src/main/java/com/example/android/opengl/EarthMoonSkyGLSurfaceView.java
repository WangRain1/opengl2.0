...
public class EarthMoonSkyGLSurfaceView extends GLSurfaceView{
    ...
    private class SceneRenderer implements Renderer
    {
        ...
        public void onSurfaceChanged(GL10 gl, int width, int height) {
            ...
            //调用此方法计算产生透视投影矩阵
            gl.glFrustumf(-ratio, ratio, -1.f, 1.f, 1f, 100);
            
            //GLU.gluLookAt(gl, 0, 0, 30.10001f, 0f, 0f, 0f, 0f, 1.0f, 0.0f);
        }
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            new Thread()
            {
                public void run()
                {
                    while(true)
                    {
                        mRenderer.earth.mAngleY+= 0.2;//球沿Y轴转动
                        mRenderer.moon.mAngleY+= 0.2;//球沿Y轴转动
                        requestRender();//重绘画面
                        try
                        {
                            Thread.sleep(25);//休息10ms再重绘
                        }
                        catch(Exception e)
                        {
                            e.printStackTrace();
    ...
    private void initSunLight(GL10 gl)
    {
        ...
        //环境光设置
        float[] ambientParams={0.15f,0.05f,0.025f,1.0f};//光参数 RGBA
        ...
        //设定光源的位置
        float[] positionParamsGreen={-14.14f,8.28f,3f,0};//最后的0表示使用定向光
